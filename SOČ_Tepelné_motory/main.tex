...
%INCLUDEONLY
%\includeonly{sections/zaver.tex}
...
\renewbibmacro*{url+urldate}{%
  \usebibmacro{url}%
  \ifboolexpr{ test {\iffieldundef{urldate}} }
    {}
    {, \bibstring{urlseen}~\printdate{urldate}}%
}

\DefineBibliographyStrings{czech}{%
  online = {[online]},  % Adds [online] before the citation
  urlseen = {Přístup dne}  % Ensures "Přístup dne" appears correctly
}
...
\newcommand{\pracovniList}[3]{
    \textbf{#2} {Dostupné ke stažení }\textattachfile[color=0 0 0]{#3}{\textit{zde}}
    \begin{minipage}{0.5\textwidth}
      \centering
      \fbox{
        \includegraphics[scale = 0.35, page=1, clip]{#1}
      }
    \end{minipage}%
    \begin{minipage}{0.5\textwidth}
      \centering
      \fbox{
        \includegraphics[scale = 0.35, page=2, clip]{#1}
      }
    \end{minipage}
}
\usepackage{tikz}
...
%aby se mezery v krátkých řádcích neroztahovaly
\raggedright
\newcommand{\odst}{\\ \vspace{\baselineskip}}
\newcommand{\podst}{\vspace{\baselineskip}}
